...
export async function GET(req: Request) {
  ...
  if (ADMIN_EMAIL) {
    ...
    const pitches = await Promise.all(
      partnerTypes.map(p =>
        ...
        Partnership idea: refer your clients to Mela and vice versa. We list them as a featured partner. No cost.
    ...
    for (const vendor of topVendors ?? []) {
      ...
      await resend.emails.send({
        ...
        html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto">
          <h2 style="color:#E8760A">Earn a free month on Mela!</h2>
          <p>${referralMsg}</p>
    ...
    await resend.emails.send({
      from: 'Mela Growth <agent@melaa.ca>',
      to: ADMIN_EMAIL,
      ...
      html: `<div style="font-family:sans-serif;max-width:650px;margin:0 auto">
        ...
        ${partnerHtml}
        <p style="color:#999;font-size:12px;margin-top:24px">Generated by Mela Growth Agent · ${now.toDateString()}</p>
      </div>`,
    })
  ...
  if (ADMIN_EMAIL) {
    ...
    150 words max. Include: what Mela is, current season relevance, CTA with the URL melaa.ca.
