    ...
    >
      ...
      >
        ...
        <div className={`relative h-52 overflow-hidden bg-gradient-to-br ${gradient}`}>
          ...
          <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
            ...
            {tierConfig && (
              <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full border"
                style={{ background: 'rgba(247,245,242,0.92)', color: 'var(--color-text)', borderColor: 'var(--color-taupe)' }}>
                {tierConfig.label}
              </span>
            )}
          </div>
          ...
          {vendor.is_verified && (
            <div className="absolute top-3 right-3 rounded-full p-1.5" style={{ background: 'rgba(247,245,242,0.92)' }} title="Verified vendor">
        ...
        <div className="p-5 flex flex-col flex-1 gap-1.5">
          {vendor.category && (
            <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--color-gold-dark)' }}>
              {vendor.category.name}
            </p>
          )}

          <h3 className="font-[family-name:var(--font-playfair)] font-bold text-base leading-snug line-clamp-1 transition-colors duration-200 group-hover:opacity-70"
          ...
          {hasRating ? (
            <div className="flex items-center gap-1.5">
              <div className="flex items-center gap-0.5">
                {[1, 2, 3, 4, 5].map(i => (
                  <Star key={i} className={`w-3 h-3 fill-current ${i <= Math.round(rating!) ? '' : 'opacity-20'}`}
                    style={{ color: 'var(--color-gold)' }} />
                ))}
              </div>
              <span className="text-xs font-bold" style={{ color: 'var(--color-text)' }}>{rating!.toFixed(1)}</span>
              <span className="text-[11px]" style={{ color: 'var(--color-taupe)' }}>({reviews})</span>
            </div>
          ) : (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border w-fit"
              style={{ color: 'var(--color-gold-dark)', borderColor: 'var(--color-gold)', background: 'var(--color-gold-light)' }}>
          ...
          {vendor.city && (
            <div className="flex items-center gap-1 text-[11px]" style={{ color: 'var(--color-taupe)' }}>
              ...
              {(vendor as { distance_km?: number }).distance_km != null && (
                <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-[#C8A96A]/10 text-[#C8A96A] shrink-0">
                  {(vendor as { distance_km: number }).distance_km < 1 ? '<1' : (vendor as { distance_km: number }).distance_km} km
                </span>
              )}
            </div>
          )}
          ...
          <div className="mt-auto pt-3 border-t flex items-center justify-between" style={{ borderColor: 'var(--color-section)' }}>
            <span className="inline-flex items-center gap-1 text-[11.5px] font-semibold group-hover:gap-2 transition-all duration-200"
              style={{ color: 'var(--color-gold-dark)' }}>
              View Profile
              <ArrowUpRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200" />
            </span>
            {vendor.website && (
              <span className="flex items-center gap-1 text-[10px] px-2 py-1 rounded-lg border transition-colors duration-150"
                style={{ color: 'var(--color-taupe)', borderColor: 'var(--color-section)', background: 'var(--color-bg)' }}>
