    ...
    <div className="relative w-full">
      <form
        ...
        className={`w-full flex items-center bg-white rounded-2xl transition-all duration-300 ${
          focused
            ? 'shadow-[0_0_0_3px_rgba(232,118,10,0.2),0_8px_32px_rgba(0,0,0,0.12)]'
            : 'shadow-[0_4px_24px_rgba(0,0,0,0.10),0_1px_4px_rgba(0,0,0,0.06)]'
        }`}
      >
        <div className="flex items-center pl-5 text-gray-400">
          <Search className={`w-5 h-5 transition-colors duration-200 ${focused ? 'text-[#E8760A]' : 'text-gray-400'}`} />
        </div>
        <input
          type="text"
          value={query}
        ...
        <button
          type="submit"
          aria-label="Search"
          className="btn-primary m-2 flex items-center gap-2 bg-[#E8760A] hover:bg-[#d06a09] text-white font-semibold px-5 py-3 rounded-xl text-sm transition-colors duration-200 shadow-saffron whitespace-nowrap"
        >
          Search
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-[0_16px_48px_rgba(0,0,0,0.12)] border border-gray-100 overflow-hidden z-50 animate-slide-down">
          ...
          {SUGGESTIONS.map((suggestion, i) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-orange-50/60 transition-colors duration-150 text-left group"
              style={{ animationDelay: `${i * 30}ms` }}
            >
              <Search className="w-3.5 h-3.5 text-gray-300 group-hover:text-[#E8760A] transition-colors shrink-0" />
              <span className="text-sm text-gray-600 group-hover:text-gray-900 transition-colors">{suggestion}</span>
              <ArrowRight className="w-3 h-3 text-gray-300 group-hover:text-[#E8760A] ml-auto opacity-0 group-hover:opacity-100 transition-all duration-150" />
            </button>
          ))}
        </div>
      )}
